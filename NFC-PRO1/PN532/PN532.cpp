...
SerialUSB.println("writeCommand1sss");
    if (HAL(writeCommand)(pn532_packetbuffer, 1)) {
        return 0;
    }
SerialUSB.println("readResponse1sss");
    // read data packet
...
SerialUSB.println("readPassiveTargetID");
    if (HAL(writeCommand)(pn532_packetbuffer, 3)) {
        return 0x0;  // command failed
    }
SerialUSB.println("writeCommand");
    // read data packet
    if (HAL(readResponse)(pn532_packetbuffer, sizeof(pn532_packetbuffer), timeout) < 0) {
        return 0x0;
    }
SerialUSB.println("readResponse");
